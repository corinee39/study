...
```
  - 인스턴스 멤버 -> 객체가 있어야만 사용할 수 있는 멤버 (위의 코드에서 gas 필드와 setSpeed() 메소드)<br/>
    => 외부 클래스에서 사용하려면? `Car 객체 생성 후 참조 변수로 접근해야함`

### this 키워드
  - `객체 내부`에서는 인스턴스 멤버에 접근하기 위해 this 키워드를 사용
  - 생성자와 메소드의 매개변수명이 인스턴스 멤버인 필드명과 동일한 경우 -> 인스턴스 필드임을 강조할 때 this 사용
    ```java
    // Car.java
    public class Car {
      // 필드 선언
    ...
    ```
## 6.10 정적 멤버
- `정적(static) 멤버`: 메소드 영역의 클래스에 고정적으로 위치하는 멤버<br/>
   -> 객체 생성할 필요없이 클래스를 통해 바로 사용 가능
...
### 정적 멤버 사용
- 클래스 이름과 함께 `도트(.) 연산자`로 정적 멤버에 접근
- 정적 필드와 정적 메소드는 객체 참조 변수로도 점근 가능 -> but 클래스 이름으로 접근하는 것이 정석
  ```java
  // Calculator.java
  public class Calculator {
...
### 인스턴스 멤버 사용 불가
- 정적 메소드와 정적 블록 -> 객체가 없어도 실행 가능 <br/>
  => 객체 내부에 인스턴스 필드나 인스턴스 메소드 사용 불가 / this도 사용 불가
- 정적 메소드와 정적 블록에서 인스턴스 멤버를 사용하고 싶으면? <br/>
  => 객체를 생성하고 참조 변수로 접근해야함
- 정적 메소드의 대표적인 예: `main()`
...
### final 필드 선언
- final 필드는 초기값이 최종값이 됨! 도중에 `수정 불가`
  - final 필드에 초기값을 주는 방법
    1. 필드 선언 시에 초기값 대입 <- 고정값일때
    2. 생성자에서 초기값 대입 <- 객체 생성 시 외부에서 전달된 값으로 초기화할 때
